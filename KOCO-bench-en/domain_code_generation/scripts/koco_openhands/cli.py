...
def cmd_memory_init(args) -> int:
    from memory.observed_memory.explore import run_explore
    ...
    _print_banner(
        "Observed Memory: Explore Code",
        Framework=args.framework,
        TestExample=args.test_example,
        Model=args.model,
    )
    try:
        out = run_explore(
...
def _memory_requires_api_key(strategy: str) -> bool:
    strategies = [s.strip() for s in strategy.split(",") if s.strip()]
    if strategy == "all":
        return True
    return any(item in {"llm", "business_logic"} for item in strategies)


def cmd_memory_select(args) -> int:
    from memory.observed_memory.function_select import run_select

    api_key = ""
    if _memory_requires_api_key(args.strategy):
        api_key = _resolve_api_key(getattr(args, "api_key", ""))
        if not api_key:
            print("ERROR: OPENROUTER_API_KEY is not set.")
            return 1
    ...
    _print_banner(
        "Observed Memory: Select Functions",
        Framework=args.framework,
        TestExample=args.test_example,
        Model=args.model,
...
def cmd_memory_generate(args) -> int:
    from memory.observed_memory.generate import run_generate
    from memory.config import candidates_path, test_input_path
    ...
    _print_banner(
        "Observed Memory: Generate Test Inputs",
        Framework=args.framework,
        TestExample=args.test_example,
        Model=args.model,
    )
...
def cmd_memory_coverage(args) -> int:
    from memory.observed_memory.coverage_orch import run_coverage
    ...
    _print_banner(
        ...
        Framework=args.framework,
        TestExample=args.test_example,
        FunctionName=getattr(args, "function_name", None) or "all",
        OutputCapture="off" if no_output else "on",
    )
    try:
        results = run_coverage(
            ...
            per_test=getattr(args, "per_test", False),
...
def cmd_memory_feedback(args) -> int:
    ...
    _print_banner(
        "Observed Memory: Coverage Feedback",
        Framework=args.framework,
        TestExample=args.test_example,
        Model=args.model,
        Threshold=f"{args.coverage_threshold:.0%}",
...
def cmd_memory_procedural(args) -> int:
    from memory.procedural_memory.run_pilot import main as procedural_main

    saved_argv = sys.argv
    procedural_args = list(getattr(args, "procedural_args", []))
    if getattr(args, "procedural_help", False) or not procedural_args:
        procedural_args = ["--help"]
    sys.argv = ["memory procedural"] + procedural_args
    try:
        result = procedural_main()
...
def main():
    ...
    def add_infer_args(sp):
        ...
        sp.add_argument("--max-iterations", type=int, default=100, help="Max agent turns (default: 100)")
        ...
        sp.add_argument(
            "--graph-knowledge-profile",
            default="",
            help="Profile-scoped graph knowledge to use during inference",
        )
        sp.add_argument(
            "--graph-knowledge-artifact",
            choices=("auto", "optimized", "mounted"),
            default="auto",
            help=(
    ...
    # observed memory
    mem = subparsers.add_parser("memory", help="Observed memory generation")
    mem_sub = mem.add_subparsers(dest="memory_command")

    m_init = mem_sub.add_parser("init", help="Explore code and generate observed memory + dep graph")
    m_init.add_argument("--framework", required=True, help="Framework name")
    m_init.add_argument("--test-example", required=True, help="Test example name")
    m_init.add_argument("--model", default="deepseek/deepseek-v3.2", help="OpenRouter model")
    m_init.add_argument("--api-key", default="", help="OpenRouter API key")
    m_init.add_argument("--base-url", default="https://openrouter.ai/api/v1", help="API base URL")
    m_init.add_argument("--max-iterations", type=int, default=100, help="Max agent turns (default: 100)")

    m_select = mem_sub.add_parser("select", help="Select candidate functions for test generation")
    m_select.add_argument("--framework", required=True, help="Framework name")
    m_select.add_argument("--test-example", required=True, help="Test example name")
    m_select.add_argument("--model", default="deepseek/deepseek-v3.2", help="OpenRouter model")
    m_select.add_argument("--api-key", default="", help="OpenRouter API key")
    m_select.add_argument("--base-url", default="https://openrouter.ai/api/v1", help="API base URL")
    ...
    m_generate = mem_sub.add_parser("generate", help="Generate requirement docs and test_input.py")
    m_generate.add_argument("--framework", required=True, help="Framework name")
    m_generate.add_argument("--test-example", required=True, help="Test example name")
    m_generate.add_argument("--model", default="deepseek/deepseek-v3.2", help="OpenRouter model")
    ...
    m_coverage = mem_sub.add_parser("coverage", help="Measure generated test_input.py coverage")
    m_coverage.add_argument("--framework", required=True, help="Framework name")
    m_coverage.add_argument("--test-example", required=True, help="Test example name")
    m_coverage.add_argument("--function-name", default=None, help="Single function (default: all candidates)")
    m_coverage.add_argument("--per-test", action="store_true", help="Per-test coverage breakdown")
    ...
    m_feedback.add_argument("--test-example", required=True, help="Test example name")
    m_feedback.add_argument("--function-name", default=None, help="Single function (default: all below threshold)")
    m_feedback.add_argument("--model", default="deepseek/deepseek-v3.2", help="OpenRouter model")
    ...
    m_proc = mem_sub.add_parser(
        "procedural",
        add_help=False,
        help="Build procedural memory from observed-memory practice runs",
    )
    m_proc.add_argument("-h", "--help", action="store_true", dest="procedural_help")
    ...
    if args.command == "memory":
        ...
        dispatch = {
            ...
            "select": cmd_memory_select,
            "generate": cmd_memory_generate,
            "coverage": cmd_memory_coverage,
            "feedback": cmd_memory_feedback,
            "procedural": cmd_memory_procedural,
        }
        return dispatch[args.memory_command](args)
    ...
    if args.command == "infer":
        _print_banner(
            ...
            Debug=args.debug,
            GraphKnowledgeProfile=args.graph_knowledge_profile or "default",
            GraphKnowledgeArtifact=args.graph_knowledge_artifact,
        )
        return cmd_infer(
